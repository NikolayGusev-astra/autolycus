...
# between `hermes tools` and `autolycus setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # on demand via `hermes auth add`, `autolycus setup` vision, and
    # `autolycus setup tts`. This keeps both quick and full setup thin.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via autolycus setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `autolycus setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `autolycus setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `autolycus setup agent` later to customize.")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  autolycus config edit")
    print_info("   Open config in your editor:  autolycus config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     autolycus config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: autolycus gateway install")
            else:
                print_info("  You can install later: autolycus gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo autolycus gateway install --system")
                print_info("  Or run in foreground:  autolycus gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   autolycus gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   autolycus gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `autolycus setup tools` and `hermes tools` use the same flow:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``autolycus setup --portal`` and ``hermes portal``. This is the
    ...
    ``autolycus setup`` and hunt for the quick-setup option.
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      autolycus setup           — full or quick (auto-detected)
      autolycus setup model     — just model/provider
      autolycus setup tts       — just text-to-speech
      autolycus setup terminal  — just terminal backend
      autolycus setup gateway   — just messaging platforms
      autolycus setup tools     — just tool configuration
      autolycus setup agent     — just agent settings
    """
    ...
    print(
        color(
            "│             ⚕ Autolycus Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Autolycus Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    # Tune later with `autolycus setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``autolycus setup <section>``
    ...
    print()
    print_info("  Configure all settings:    autolycus setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  autolycus setup gateway")
    print()
