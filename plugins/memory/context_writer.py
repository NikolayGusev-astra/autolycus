...
class ContextWriter:
    ...
    def summarize_context(self, session_id: str,
                          summary_text: str | None = None,
                          turn_files: list[Path] | None = None) -> str:
        """Суммировать все raw turn'ы сессии в итоговый wiki-документ.
        
        Args:
            session_id: ID сессии
            summary_text: Итоговый текст (опционально — если есть LLM-summary).
                          Если None — собирает все raw turn'ы в один файл.
            turn_files: Конкретные файлы для суммирования (опционально).
                        Если None — все файлы сессии.
            
        Returns:
            Путь к созданному wiki-документу.
        """
        session_dir = self._session_dir(session_id)
        if not session_dir.exists():
            raise FileNotFoundError(f"Session {session_id} has no context files")
        
        files = turn_files or sorted(session_dir.glob("turn_*.md"))
        if not files:
            raise FileNotFoundError(f"No context files for session {session_id}")
        
        # Собираем контент
        turns_count = len(files)
        raw_parts = []
        for f in files:
        ...
        summary = summary_text or "Сводка сессии — см. raw turn'ы в деталях"
...
**Turns:** {turns_count}
**Archive size:** {sum(f.stat().st_size for f in files) // 1024} KB
...
"""
        # Сохраняем
        dest_dir = self.wiki_dir / "concepts"
        dest_dir.mkdir(parents=True, exist_ok=True)
        dest_path = dest_dir / f"{session_id}-context.md"
        dest_path.write_text(doc)
        
        logger.info("[ContextWriter] summarized %d turns → %s", turns_count, dest_path)
        return str(dest_path)
    
    def compress_session(self, session_id: str, keep_turns: int = 10,
                         summary_text: str | None = None) -> str:
        """Compaction: суммировать старые turn'ы, оставить active window.
        
        Args:
            session_id: ID сессии
            keep_turns: Сколько turn'ов оставить в raw
            summary_text: Итоговый текст (опционально)
        ...
        files = sorted(session_dir.glob("turn_*.md"))
        if len(files) <= keep_turns:
            return ""  # Нечего суммировать
        
        # Старые turn'ы (первые N - keep)
        old_files = files[:-(keep_turns or self.window_size)]
        
        # Суммируем их
        result = self.summarize_context(
            session_id=f"{session_id}-archive-{old_files[0].stem}-{old_files[-1].stem}",
            summary_text=summary_text,
            turn_files=old_files,
        )
        
        # Удаляем старые raw-файлы
        for f in old_files:
            f.unlink()
        
        logger.info("[ContextWriter] compressed %d/%d turns → %s",
                    len(old_files), len(files), result)
        return result
