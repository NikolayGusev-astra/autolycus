...
class TelegramAdapter(BasePlatformAdapter):
    ...
    # ── Workflow classification (autolycus) ────────────────────────────────

    _workflow_classifier = None

    @classmethod
    def _get_workflow_classifier(cls):
        if cls._workflow_classifier is not None:
            return cls._workflow_classifier
        try:
            from scripts.workflow_classifier import WorkflowClassifier
            cls._workflow_classifier = WorkflowClassifier()
        except Exception:
            cls._workflow_classifier = None
        return cls._workflow_classifier
